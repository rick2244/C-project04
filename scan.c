...
bool scan_is_hex(char c){
	return scan_is_digit(c) || (c >= 'a' && c<= 'f') || (c >= 'A' && c <= 'F');
}
...
char * scan_token(char *p, char *end, struct scan_token_st *tp) {
    if (p == end) {
    	...
    	p = scan_read_token(tp, p, 2, TK_FOR);
    } else if(*p == '<' && *(p+1) == '<'){
    	p = scan_read_token(tp, p, 2, TK_BACK);
    } else if(*p == '~'){
    	...
    	p = scan_read_token(tp, p, 1, TK_EXOR);
    }
    else {
